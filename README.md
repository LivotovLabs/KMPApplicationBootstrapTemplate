# KMP Application Bootstrap (Composables UI Edition)

This is the **Composables UI** version of the KMP project template. It replaces Material 3 with the **[Composables UI Kit](https://composables.com)** (Composables Core) for a more flexible, unstyled foundation.
...
*   **Design System**: Custom implementation using `CompositionLocal` (see `ui/design`).
*   **Icons**: Lucide Icons via `com.composables:icons-lucide-cmp`.
*   **Architecture**: VISCE with OSKit-KMP.
...
### Version 1.1 (Composables UI)
*   **Initial Composables UI Release**: Migrated the template from Material 3 to Composables Core.
*   **Custom Design System**: Added `AppTheme`, `AppColors`, `AppTypography`, and `AppDimensions` in `ui/design`.
*   **Lucide Icons**: Integrated the Lucide icon library for modern, lightweight icons.
*   **AppBaseButton**: Added a demonstration of a custom button built with `com.composables.core.Clickable`.
*   **Fixed Android App Icon Resolution**: (Same as main branch) Removed conflicting default icons.
